...
plot_prefix <- paste0(dataset, "_", grouping, "_", location, "_", ifelse(neuron_as_sender, "neuron_send", "neuron_receive"))
...
cornea_idents <- levels(cellchat@idents) %>% purrr::keep(function(x) grepl("cornea_", x, ignore.case = TRUE))
tg_idents <- levels(cellchat@idents) %>% purrr::keep(function(x) grepl("tg_", x, ignore.case = TRUE))

non_neuron_idents <- if("tg" == location) tg_idents else cornea_idents
...
if (TRUE == neuron_as_sender) {
  ...
  target_groups <- match(non_neuron_idents, levels(cellchat@idents))
  
} else {
  
  # Interested in Neuron as receiver
  source_groups <- match(non_neuron_idents, levels(cellchat@idents))
...
circle_plot <- 
  netVisual_circle(cellchat@net$weight, 
                   idents.use = c("neuron", non_neuron_idents),
...
svg(paste0(figures_dir, plot_prefix, "circle_plot.svg"), width = 15, height = 14)
circle_plot
dev.off()
...
svg(paste0(figures_dir, plot_prefix, "pathway_heatmap.svg"), width = 10, height = 7)
...
ggsave(paste0(figures_dir, plot_prefix, "bubbles_select_pathways.svg"), plot = last_plot(), width = 35, height = 10)
...
control_data <- SeuratDisk::LoadH5Seurat(paste0(data_dir, "seurat/control_with_neurons.h5Seurat"))  
...
#######################################################################################################################################
### Create the Control TG + Cornea + Neurons object [SHOULD ONLY NEED TO RUN ONCE]

control_cornea <- SeuratDisk::LoadH5Seurat(paste0(data_dir, "seurat/sept_cornea_annot.h5Seurat")) %>% subset(subset = "control" == condition)
control_tg <- SeuratDisk::LoadH5Seurat(paste0(data_dir, "seurat/combined_tg_annot.h5Seurat")) %>% subset(subset = "control" == condition)
control_neurons <- SeuratDisk::LoadH5Seurat(paste0(data_dir, "seurat/neurons_all_conditions.h5Seurat")) %>% subset(subset = "control" == condition)

seurat_obj <- merge(control_cornea, list(control_tg, control_neurons), merge.data = TRUE)
...
seurat_obj$celltypeloc <- factor(ifelse("neuron" == seurat_obj$cell_L1, "neuron", 
                                        paste0(seurat_obj$location, "_", seurat_obj$cell_L2)))

seurat_obj %>% SeuratDisk::SaveH5Seurat(paste0(data_dir, "seurat/control_with_neurons.h5Seurat"))
