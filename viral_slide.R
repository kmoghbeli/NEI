...
seurat_obj <- SeuratDisk::LoadH5Seurat(paste0(data_dir, "seurat/combined_tg_viral_annot.h5Seurat"))

hsv_genes <- rownames(seurat_obj)[str_starts(rownames(seurat_obj), "HSV-")]

#TODO: ADD "keep_genes" to send HSV genes
data_subset <- subsetNIData(seurat_obj, 
                            celltype_metadata_key =  "cell_L2", 
                            celltype_metadata_val = "Mye", 
                            conditions = c("kos", "re"), 
                            exclude_mito_ribo_genes = TRUE,
                            genes_to_keep = hsv_genes)

hsv_genes_in_subset <- colnames(data_subset)[str_starts(colnames(seurat_obj), "HSV-")]
